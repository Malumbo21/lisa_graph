...
from django.db.models import F, Value, Max, Q
...
class Query(graphene.ObjectType):
	...
	top_movers = graphene.List(StockDataV2Type, n=graphene.Int(), net=graphene.Boolean(), day=graphene.Boolean(), week=graphene.Boolean(), month=graphene.Boolean(), year=graphene.Boolean())
	top_losers = graphene.List(StockDataV2Type, n=graphene.Int(), net=graphene.Boolean(), day=graphene.Boolean(), week=graphene.Boolean(), month=graphene.Boolean(), year=graphene.Boolean())
	...
	def resolve_top_movers(self, info, n=None, net=False, day=False, week=False, month=False, year=False):
		...
		if net:
			data = data.order_by('-net_change')
		else:
			data = data.order_by('-change')

		if n is not None:
			return data[:n]
		else:
			return data[:5]
	def resolve_top_losers(self, info, n=None, net=False, day=False, week=False, month=False, year=False):
		if [day, week, month, year] == [False, False, False, False]:
			raise Exception("Error set one on the timeframe options (day, week, month, year) to True")
		if day == True:
			date = StockDataV2.objects.aggregate(Max('date'))['date__max']
